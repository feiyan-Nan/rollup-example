...
const alwaysNewT = (i18n, language, namespace, keyPrefix, showI18nKey) => (key, opts, ...rest) => showI18nKey ? `${key} : ${i18n.getFixedT(language, namespace, keyPrefix)(key, opts, ...rest)}`: i18n.getFixedT(language, namespace, keyPrefix)(key, opts, ...rest);

const useMemoizedT = (i18n, language, namespace, keyPrefix, showI18nKey) =>
  useCallback(alwaysNewT(i18n, language, namespace, keyPrefix, showI18nKey), [
    i18n,
    language,
    namespace,
    keyPrefix,
...
const sparrowI18nKey = 'sparrow-show-i18n';

export const useTranslation = (ns, props = {}) => {
  // assert we have the needed i18nInstance
  const { i18n: i18nFromProps } = props;
  const { i18n: i18nFromContext, defaultNS: defaultNSFromContext } = useContext(I18nContext) || {};
  const [showI18nKey, setShowI18nKey] = useState(localStorage.getItem(sparrowI18nKey) === 'true');
  ...
  const handleMessage =  (event) => {
    if (event.data.type === sparrowI18nKey) {
      const { isShowI18nKey } = event?.data?.value ?? {};
      setShowI18nKey(isShowI18nKey);
    }
  }
  const handleStorageChange = (event) => {
    if (event.key === sparrowI18nKey) {
      const isShowI18nKey = event?.newValue === 'true';
      setShowI18nKey(isShowI18nKey);
    }
  }
  useEffect(() => {
    // 通过监听message事件拿到用户切换 sparrowI18nKey 的状态
    window.addEventListener('message', handleMessage)
    // 由于appStore iframe的貌似被劫持过 目前拿不到chrome插件下发的 postmessage 的事件
    // 这里兜底监听 localStorage 的变化 来获取用户切换 sparrowI18nKey 的状态
